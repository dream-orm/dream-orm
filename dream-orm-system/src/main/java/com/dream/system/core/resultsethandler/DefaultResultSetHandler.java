...
public class DefaultResultSetHandler implements ResultSetHandler {
    ...
    protected MappedResult createMappedResult(ResultSet resultSet, MappedStatement mappedStatement) throws SQLException {
        ...
        for (int i = 1; i <= columnCount; i++) {
            ...
            MappedColumn.Builder builder = new MappedColumn.Builder().index(i).jdbcType(jdbcType).table(tableName).columnLabel(columnLabel).columnInfo(columnInfo);
            try {
                ...
                if (!SystemUtil.ignoreField(field)) {
                    ...
                    if (mappingField) {
                        ...
                        if ((equal = fieldName.equals(columnLabel)) || fieldName.equalsIgnoreCase(columnLabel)) {
                            ...
                            builder.typeHandler(typeHandlerFactory.getTypeHandler(field.getType(), builder.getJdbcType()));
                            builder.field(field);
